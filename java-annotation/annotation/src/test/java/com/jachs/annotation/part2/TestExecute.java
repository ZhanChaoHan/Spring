		...
		for (Class cls : clList) {
			for (Annotation class1 : cls.getAnnotations()) {
				if(cls.isAnnotationPresent(ExecuteSQL.class)) {//判断注解类型是否是ExcuteSQL
					...
					executeMethod(eSQL.driverClass(), eSQL.userName(), eSQL.passWorld(), eSQL.connectURL(), eSQL.excuteSQLText());
				}
			}
		}
	}
	private static  void executeMethod(String driverClass,String userName,String passWorld,String connectURL,String excuteSQLText) {
