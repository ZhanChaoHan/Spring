package com.jachs.annotation.processor.utill;
...
public final class JavaWriter {
	...
	/**
	 * @param out the stream to which Java source will be written. This should be a
	 *            buffered stream.
	 */
	...
	/**
	 * Emit an import for {@code type}. For the duration of the file, all references
	 * to this class will be automatically shortened.
	 */
	 ...
	 * java.util.List<java.lang.String>}, shorting it with imports if possible.
	 */
	 ...
	 */
	public void beginType(String type, String kind, int modifiers, String extendsType, String... implementsTypes)
			throws IOException {
		indent();
		modifiers(modifiers);
		out.write(kind);
	...
	public void field(String type, String name, int modifiers, String initialValue) throws IOException {
		indent();
		modifiers(modifiers);
		type(type);
		out.write(" ");
	 ...
	 *                   constructors.
	 */
	public void beginMethod(String returnType, String name, int modifiers, String... parameters) throws IOException {
		indent();
		modifiers(modifiers);
		if (returnType != null) {
			type(returnType);
	...
	/**
	 * @param pattern a code pattern like "int i = %s". Shouldn't contain a trailing
	...
	/**
	 * @param controlFlow the control flow construct and its code, such as "if (foo
	...
	/**
	 * @param controlFlow the control flow construct and its code, such as "else if
	...
	private enum Scope {
		TYPE_DECLARATION, ABSTRACT_METHOD, NON_ABSTRACT_METHOD, CONTROL_FLOW,
	}
}
